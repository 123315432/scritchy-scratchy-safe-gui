-- subscription_bot_fields.lua - whitelist read/write Subscription Bot upgrade counts in LayerOne.upgradeDataDict
...
local MODE = tostring(SCRITCHY_SUB_BOT_MODE or 'status')
if MODE ~= 'status' and MODE ~= 'apply' then return 'ERR: SCRITCHY_SUB_BOT_MODE must be status/apply, got=' .. MODE end

local TARGET_NAMES = {'Subscription Bot', 'Buying Speed'}
local TARGETS = {
...
local out = {string.format('subscription bot upgrades saveData=%s source=%s layerOne=%s dict=%s mode=%s max_source=%s', hx(saveData), tostring(source), hx(layerOne), hx(dict), MODE, MAX_SOURCE)}
local seen = {}
for i=0,arrlen-1 do
